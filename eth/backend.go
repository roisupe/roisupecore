...
func New(sctx *node.ServiceContext, config *Config) (*GoChain, error) {
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run eth.Roisupe in light sync mode, use les.LightRoisupe")
	}
	...
	log.Info("Initialising Roisupe protocol", "versions", ProtocolVersions, "network", config.NetworkId, "dbversion", dbVer)
