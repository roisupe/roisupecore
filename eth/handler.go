...
import (
	...
	"github.com/gochain/gochain/v3/eth/fetcher"
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Roisupe protocol")
	...
	log.Info("Roisupe protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Roisupe peer connected", "name", p.Name())
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Error("Roisupe message handling failed", "err", err)
			return err
		}
	}
}
