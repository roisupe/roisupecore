...
const (
	clientIdentifier = "Roisupe" // Client identifier to advertise over the network
)

var (
	// Git SHA1 commit hash of the release (set via linker flags)
	gitCommit = ""
	// Roisupe address of the Roisupe release oracle.
	relOracle = common.HexToAddress("0xfa7b9770ca4cb04296cac84f37736d4041251cdf")
	// The app that holds all commands and flags.
	app = utils.NewApp(gitCommit, "the Roisupe command line interface")
...
func init() {
	// Initialize the CLI app and start Roisupe
	app.Action = gochain
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2022 The Roisupe Authors"
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	stack.AccountManager().Subscribe(events, "Roisupe-startNode")
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) || ctx.GlobalBool(utils.LocalFlag.Name) {
		...
		if err := stack.Service(&gochain); err != nil {
			utils.Fatalf("Roisupe service not running: %v", err)
		}
